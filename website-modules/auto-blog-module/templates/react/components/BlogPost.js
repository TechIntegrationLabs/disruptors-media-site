...
const BlogPost = () => {
  ...
  const fetchPost = async () => {
    try {
      ...
      if (sheetPost) {
        ...
        if (sheetPost.postUrl || sheetPost.content) {
          fetchPostContent(sheetPost.postUrl || sheetPost.content);
        } else {
  ...
  const fetchPostContent = async (contentUrl) => {
    try {
      setContentLoading(true);
      const fetchedContent = await fetchGoogleDocsContent(contentUrl);
      setContent(fetchedContent);
    } catch (error) {
      console.error('Error fetching post content:', error);
      // Set a fallback content
      setContent({
        html: '<p class="blog-paragraph">Content is currently being loaded. Please check back soon.</p>',
        plainText: 'Content is currently being loaded. Please check back soon.'
      });
    } finally {
      setContentLoading(false);
    }
    ...
    <div className="blog-post-page">
      ...
      <section className="blog-post-hero">
        <div className="blog-post-hero-image">
          <img src={getOptimizedImageUrl(post.image, 1200, 600)} alt={post.title} />
            ...
            <article className="blog-post-content">
              {contentLoading ? (
                ...
                <div 
                  ...
                  dangerouslySetInnerHTML={{ __html: content?.html || post.content || '<p>Content not available</p>' }}
                />
              )}
